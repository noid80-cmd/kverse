      ...
      <div className="max-w-lg mx-auto px-4 pt-10">
        ...
        <div style={{ borderRadius: 20, overflow: 'hidden', background: '#000', marginBottom: 20, boxShadow: '0 8px 32px rgba(0,0,0,0.4)' }}>
          {video.video_url ? (
            ...
            <div style={{ height: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#8A7F6E', fontSize: 14 }}>
              {tx.videos.videoLoading}
            </div>
          )}
        </div>

        <div style={{ background: '#FFFFFF', borderRadius: 20, padding: 20, border: '1px solid rgba(36,28,21,0.09)', marginBottom: 16 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12, flexWrap: 'wrap' }}>
            <span style={{ fontSize: 12, background: 'rgba(36,28,21,0.09)', color: '#aaaacc', padding: '4px 10px', borderRadius: 8, fontWeight: 700 }}>
              {categoryLabel[video.category] ?? video.category}
            </span>
            <span style={{ fontSize: 12, color: '#8A7F6E' }}>{video.view_count} {tx.videos.views}</span>
            <span style={{ fontSize: 12, color: '#8A7F6E', fontWeight: 600 }}>
              {video.status === 'active' ? tx.videos.statusActive : tx.videos.statusReview}
            </span>
          </div>
          {video.description && <p style={{ fontSize: 14, color: '#8A7F6E', lineHeight: 1.6, marginBottom: 12 }}>{video.description}</p>}
          {video.tags.length > 0 && (
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
              {video.tags.map(t => (
                <span key={t} style={{ fontSize: 12, background: 'rgba(36,28,21,0.09)', color: '#8A7F6E', padding: '3px 10px', borderRadius: 20, fontWeight: 600 }}>
                  #{t}
                </span>
              ))}
            </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 16 }}>
          <button onClick={toggleLike}
            ...
            <Heart size={20} strokeWidth={1.8} color={liked ? '#f43f5e' : '#8A7F6E'} fill={liked ? '#f43f5e' : 'none'} />
            <div>
              <div style={{ fontSize: 22, fontWeight: 900, color: '#241C15' }}>{likeCount}</div>
              <div style={{ fontSize: 12, color: '#8A7F6E', fontWeight: 600 }}>{tx.videos.likes}</div>
            </div>
          </button>
          <div style={{ background: '#FFFFFF', border: '1px solid rgba(36,28,21,0.09)', borderRadius: 20, padding: '18px 20px', display: 'flex', alignItems: 'center', gap: 12 }}>
            <Bookmark size={20} strokeWidth={1.8} color={bookmarkCount > 0 ? '#241C15' : '#8A7F6E'} fill={bookmarkCount > 0 ? '#241C15' : 'none'} />
            <div>
              <div style={{ fontSize: 22, fontWeight: 900, color: '#241C15' }}>{bookmarkCount}</div>
              <div style={{ fontSize: 12, color: '#8A7F6E', fontWeight: 600 }}>{tx.videos.agencyInterest}</div>
            </div>
          </div>
        </div>
                ...
                return (
                  <button key={v} type="button" onClick={() => handleVisibilityChange(v)} disabled={savingVisibility}
                    style={{
                      ...
                      border: selected ? 'none' : '1.5px solid rgba(36,28,21,0.13)',
        ...
        {isOwner && (
          <button onClick={handleDelete}
            style={{ width: '100%', padding: '14px', borderRadius: 14, background: 'none', border: '1px solid rgba(248,113,113,0.3)', color: '#DC2626', fontWeight: 700, fontSize: 15 }}>
