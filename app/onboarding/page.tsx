...
function OnboardingContent() {
  ...
  if (step === null) return (
    <div style={{ minHeight: '100vh', background: '#FFF8E7', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ width: 32, height: 32, borderRadius: '50%', border: '3px solid rgba(36,28,21,0.1)', borderTop: '3px solid #D84A1E', animation: 'spin 0.8s linear infinite' }} />
      <style>{`@keyframes spin{to{transform:rotate(360deg)}}`}</style>
    </div>
  )

  return (
    <div style={{ minHeight: '100vh', background: '#FFF8E7', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '0 32px' }}>

      {/* 진행 표시 */}
      <div style={{ display: 'flex', gap: 6, marginBottom: 48 }}>
        {[0, 1, 2].map(s => (
          <div key={s} style={{ width: s === step ? 20 : 6, height: 6, borderRadius: 3, background: s === step ? '#D84A1E' : 'rgba(36,28,21,0.16)', transition: 'all 0.3s' }} />
        ))}
      </div>
      ...
      <div style={{ width: '100%', maxWidth: 340, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        ...
        {step === 0 && (
          <>
            <div style={{ width: 80, height: 80, borderRadius: 24, background: 'linear-gradient(135deg, #D84A1E, #FF6F3C)', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 28, boxShadow: '0 8px 32px rgba(255,111,60,0.3)' }}>
            ...
            <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 32 }}>
              {(isAgency
                ...
                <div key={item.text} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px 16px', background: '#FFFFFF', borderRadius: 14, border: '1px solid rgba(36,28,21,0.09)' }}>
                  <item.Icon size={18} color="#D84A1E" strokeWidth={1.8} style={{ flexShrink: 0 }} />
                  <span style={{ fontSize: 14, color: '#8A7F6E' }}>{item.text}</span>
                </div>
              ))}
            </div>
        ...
        {step === 1 && (
          <>
            ...
            {isIOS && (
              <div style={{ width: '100%', background: '#FFFFFF', borderRadius: 20, padding: '20px 22px', border: '1px solid rgba(36,28,21,0.1)', marginBottom: 24 }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: 14, marginBottom: 16 }}>
                  <div style={{ width: 36, height: 36, borderRadius: 10, background: 'rgba(255,111,60,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    {/* iOS 공유 아이콘 */}
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#D84A1E" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
