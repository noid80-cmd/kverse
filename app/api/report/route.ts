...
async function notifyAdmin(title: string, body: string) {
  ...
  const { data: subs } = await db
    .from('push_subscriptions').select('endpoint, p256dh, auth').eq('account_id', (adminAccount as any).id)
...
export async function POST(req: NextRequest) {
  try {
    ...
    const videoTitle = (video as any)?.title || '영상'
    const uploaderUsername = (video as any)?.accounts?.username || '알 수 없음'

    await notifyAdmin('🚨 영상 신고 접수', `@${uploaderUsername}의 "${videoTitle}" — ${reason}`)
