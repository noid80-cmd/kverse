    ...
    <div className="min-h-screen pb-28" style={{ background: '#FFF8E7' }}>
      ...
      <div className="max-w-lg mx-auto px-4 pt-10">
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 6 }}>
          ...
          <h1 style={{ fontSize: 24, fontWeight: 900, color: '#241C15', flex: 1 }}>{tx.reactions.pageTitle}</h1>
          <button onClick={() => setNotifModal(true)} style={{
            width: 40, height: 40, borderRadius: 12, display: 'flex', alignItems: 'center', justifyContent: 'center',
            background: '#FFFFFF', border: '1px solid rgba(36,28,21,0.1)', cursor: 'pointer', flexShrink: 0,
            color: notifPerm === 'granted' ? '#D84A1E' : notifPerm === 'denied' ? '#DC2626' : '#8A7F6E',
          }}>
          ...
          {(['contacts', 'bookmarks'] as const).map(t => (
            <button key={t} onClick={() => setTab(t)}
              style={{
                ...
                background: tab === t ? 'linear-gradient(135deg, #D84A1E, #FF6F3C)' : 'transparent',
        ...
        {tab === 'contacts' ? (
          convs.length === 0 ? (
            <div style={{ background: '#FFFFFF', borderRadius: 20, padding: 40, textAlign: 'center', border: '1.5px dashed rgba(36,28,21,0.1)' }}>
              ...
              <div style={{ fontWeight: 700, color: '#241C15', marginBottom: 4 }}>{tx.reactions.noChats}</div>
              <div style={{ fontSize: 13, color: '#8A7F6E' }}>{tx.reactions.noChatsDesc}</div>
            </div>
          ) : (
                  ...
                  <button onClick={() => router.push(`/chat/${c.id}`)}
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 3 }}>
                        <span style={{ fontWeight: 700, color: '#241C15', fontSize: 15 }}>{c.agency_member?.name ?? tx.auditions.agencyLabel}</span>
                      ...
                      <div style={{ fontSize: 13, color: '#8A7F6E', overflow: 'hidden', whiteSpace: 'nowrap', textOverflow: 'ellipsis' }}>
                        {c.lastMessage ?? tx.reactions.startChat}
                      </div>
                    </div>
                    <svg width="7" height="12" viewBox="0 0 7 12" fill="none"><path d="M1 1l5 5-5 5" stroke="#C9B79E" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>
                  </button>
                  <button onClick={() => deleteConv(c.id)}
                    style={{ padding: '16px', background: 'none', border: 'none', cursor: 'pointer', color: '#C9B79E', flexShrink: 0, display: 'flex', alignItems: 'center' }}>
          ...
          bookmarks.length === 0 ? (
            <div style={{ background: '#FFFFFF', borderRadius: 20, padding: 40, textAlign: 'center', border: '1.5px dashed rgba(36,28,21,0.1)' }}>
              ...
              <div style={{ fontWeight: 700, color: '#241C15', marginBottom: 4 }}>{tx.reactions.noBookmarksYet}</div>
            </div>
          ) : (
                ...
                <Link key={b.id} href={b.video ? `/videos/${b.video.id}` : '#'} style={{ textDecoration: 'none' }}>
                  <div style={{ background: '#FFFFFF', borderRadius: 18, padding: '16px 20px', border: '1px solid rgba(168,85,247,0.1)' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                      ...
                      <span style={{ fontSize: 12, color: '#8A7F6E', marginLeft: 'auto' }}>{new Date(b.created_at).toLocaleDateString('ko-KR')}</span>
                    </div>
                    {b.video && (
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, color: '#D84A1E', fontWeight: 600 }}>
                        <Video size={13} strokeWidth={2} />
                        {b.video.title}
                      </div>
                    )}
                    {b.note && <div style={{ fontSize: 13, color: '#8A7F6E', marginTop: 6, fontStyle: 'italic' }}>{b.note}</div>}
                  </div>
                </Link>
              ))}
            </div>
