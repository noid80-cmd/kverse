  ...
  return (
    <div className="min-h-screen pb-28" style={{ background: '#FFF8E7' }}>
      <div className="max-w-lg mx-auto px-4 pt-10">
        ...
        <h1 style={{ fontSize: 24, fontWeight: 900, color: '#241C15', marginBottom: 6 }}>관심 지망생</h1>
        <p style={{ fontSize: 13, color: '#8A7F6E', marginBottom: 20 }}>북마크한 영상 {bookmarks.length}개</p>
        ...
        {loading ? (
          ...
          <div style={{ background: '#FFFFFF', borderRadius: 20, padding: 40, textAlign: 'center', border: '2px dashed rgba(36,28,21,0.1)' }}>
            ...
            <div style={{ fontWeight: 700, color: '#241C15', marginBottom: 6 }}>관심 지망생이 없어요</div>
            <div style={{ fontSize: 13, color: '#8A7F6E', marginBottom: 20 }}>영상 탐색에서 별표를 눌러 저장해보세요</div>
          ...
          <div className="flex flex-col gap-3">
            {bookmarks.map(b => (
              <div key={b.id} style={{ background: '#FFFFFF', borderRadius: 20, overflow: 'hidden', border: '1px solid rgba(36,28,21,0.09)' }}>
                <div style={{ display: 'flex', alignItems: 'stretch' }}>
                  <Link href={b.video?.id ? `/agency/discover/${b.video.id}` : '#'} style={{ width: 90, flexShrink: 0, textDecoration: 'none', background: b.video?.thumbnail_url ? 'transparent' : 'rgba(255,111,60,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
                  ...
                  <div style={{ flex: 1, padding: '14px 14px 14px 16px', minWidth: 0 }}>
                    <Link href={b.talent?.id ? `/agency/talents/${b.talent.id}` : '#'} style={{ textDecoration: 'none', display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                      ...
                      <span style={{ fontWeight: 700, color: '#241C15', fontSize: 14 }}>{b.talent?.name}</span>
                      <span style={{ fontSize: 11, color: '#D84A1E', marginLeft: 2 }}>프로필 →</span>
                      {b.video?.category && (
                        <span style={{ fontSize: 11, background: 'rgba(255,111,60,0.12)', color: '#D84A1E', padding: '2px 7px', borderRadius: 6, fontWeight: 600, marginLeft: 'auto' }}>
                          {categoryLabel[b.video.category]}
                        </span>
                      )}
                    </Link>
                    <Link href={b.video?.id ? `/agency/discover/${b.video.id}` : '#'} style={{ textDecoration: 'none' }}>
                      <div style={{ fontSize: 13, color: '#8A7F6E', fontWeight: 600, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', marginBottom: 4 }}>
                        {b.video?.title ?? '영상'}
                      </div>
                    </Link>
                    {b.talent?.skills && b.talent.skills.length > 0 && (
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
                        {b.talent.skills.slice(0, 3).map(s => (
                          <span key={s} style={{ fontSize: 11, background: 'rgba(216,74,30,0.12)', color: '#a78bfa', padding: '2px 7px', borderRadius: 10, fontWeight: 600 }}>{s}</span>
                        ))}
                      </div>
                    )}
                  </div>
                </div>
                <div style={{ borderTop: '1px solid rgba(36,28,21,0.09)', padding: '10px 16px', display: 'flex', justifyContent: 'flex-end' }}>
                  <button onClick={() => removeBookmark(b.id)}
                    style={{ fontSize: 12, color: '#8A7F6E', background: 'none', border: 'none', fontWeight: 600 }}>
                    북마크 취소
                  </button>
                </div>
              </div>
